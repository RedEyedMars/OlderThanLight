...
public class PlatformMode extends OverheadMode{
	...
	public void setup(Game game, boolean colourToControl, GraphicEntity wildWall, GameEndState gameEndState){
		...
		focused = (Hero) Hub.map.getEntity(Map.ID_BLACK_HERO);
		wild = (Hero) Hub.map.getEntity(Map.ID_WHITE_HERO);
	...
	private void flipView(){
		...
		focused = wild;
		wild = temp;
		((Map)Hub.map).setVisibleSquares(focused.isBlack()?Hero.BLACK_INT:Hero.WHITE_INT);
