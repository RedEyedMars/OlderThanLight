...
import com.rem.otl.editor.Editor;
...
public class Square extends GraphicEntity implements Colourable, Saveable{
	...
	public Square(int actionType, int shapeType, int blackColour,int whiteColour, Iterator<Integer> ints, Iterator<Float> floats) {
		...
		reposition(((Map)Hub.map).getFloatCoordinate(ints.next(),Map.X_axis),
				   ((Map)Hub.map).getFloatCoordinate(ints.next(),Map.Y_axis));
		resize(((Map)Hub.map).getFloatCoordinate(ints.next(),Map.X_axis),
				((Map)Hub.map).getFloatCoordinate(ints.next(),Map.Y_axis));
	...
	public void saveTo(List<Object> toSave) {
		...
		toSave.add(((Map)Hub.map).getIntCoordinate(getX(),Map.X_axis));
		toSave.add(((Map)Hub.map).getIntCoordinate(getY(),Map.Y_axis));
		toSave.add(((Map)Hub.map).getIntCoordinate(getWidth(),Map.X_axis));
		toSave.add(((Map)Hub.map).getIntCoordinate(getHeight(),Map.Y_axis));
		saveActions(toSave);
	...
	public static Iterator<Integer> makeInts(Editor editor,
		...
		if(!onCreateAction&&!programAction){
			if(squareAction1==-1&&squareAction2==-1&&updateAction.size()==0){
				...
				ints.add(colour);
				ints.add(colour2);
				ints.add(((Map)Hub.map).getIntCoordinate(x, Map.X_axis));
				ints.add(((Map)Hub.map).getIntCoordinate(y, Map.Y_axis));
				ints.add(w);
				ints.add(h);
			}
	...
	public static void addArgsFromSquare(Square square, final List<Integer> ints, final List<Float> floats){
		List<Object> probe = new ArrayList<Object>(){
			private static final long serialVersionUID = 7833589576758926530L;
	...
	public void changeColour(Integer blackColour, Integer whiteColour) {
		...
		displayFor(((Map)Hub.map).getVisibleColour());
	}
