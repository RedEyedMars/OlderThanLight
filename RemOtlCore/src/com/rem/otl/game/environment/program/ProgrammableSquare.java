...
import com.rem.otl.game.environment.onstep.NullOnStepAction;
...
public class ProgrammableSquare extends UpdatableSquare implements Creatable{
	...
	protected void loadActions(Iterator<Integer> ints, Iterator<Float> floats){
		...
		this.blackAction = new NullOnStepAction(){
			...
			public void setTarget(Object target){
				if(currentBlackAction!=null){
					((OnStepAction<Object>)currentBlackAction).setTarget(target);
				}
			}
			@Override
		...
		this.whiteAction = new NullOnStepAction(){
			...
			public void setTarget(Object target){
				if(currentWhiteAction!=null){
					((OnStepAction<Object>)currentWhiteAction).setTarget(target);				
				}
			}
			@Override
			public int targetType(){
	...
	public Object getSubject(int type, DataHolder holder) {
		switch(type){
		case 0:	return this;
		case 1: return Hub.map.getEntity(Map.ID_BLACK_HERO);
		case 2: return Hub.map.getEntity(Map.ID_WHITE_HERO);		
		...
		case 7: {
			...
			if(!knownSquares.containsKey(knownIndex)){
				Square square = (Square) Hub.map.getEntity(Map.getIdFromMult(Map.MULT_ID_TEMPLATE_SQUARE, ((Integer)holder.getData("templateIndex"))));
