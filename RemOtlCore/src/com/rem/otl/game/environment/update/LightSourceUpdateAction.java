...
public class LightSourceUpdateAction extends UpdateAction{
	...
	public void act(Double seconds) {
		...
		if(horizontal){
			if(blackLight==null){
				...
				Hub.map.addEntity(Map.ID_DISPLAY_SQUARE,blackLight);
				Hub.map.addEntity(Map.ID_DISPLAY_SQUARE,whiteLight);
		...
		if(self.getColour(colour)!=-1&&
			...
			if(show){
				if(reflector==null){
					...
					Hub.map.addEntity(Map.ID_DISPLAY_SQUARE,reflector);
	...
	private GraphicEntity getClosest(boolean colour, boolean horizontal){
		List<OnStepSquare> squares = (List<OnStepSquare>)Hub.map.getEntity(Map.ID_FUNCTIONAL_SQUARES);
