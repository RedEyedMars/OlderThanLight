...
import com.rem.otl.game.environment.Colourable;
import com.rem.otl.game.environment.Map;
import com.rem.otl.game.environment.Square;
import com.rem.otl.game.environment.onstep.HazardOnStepAction;
		...
		for(int i=mapSquares.size()-1;i>=0;--i){
			...
			if(action!=null){
				if(this.isWithin(mapSquares.get(i))){
					if((Boolean) Hub.map.getEntity(Map.ID_IS_LIGHT_DEPENDENT)){
