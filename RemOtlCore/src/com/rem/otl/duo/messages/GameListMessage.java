...
public class GameListMessage extends Message{
	...
	/**
	 * Tells this {@link com.rem.otl.duo.client.Client} to update its game list using the variables stored in this {@link com.rem.otl.duo.messages.Message}.
	 */
	@Override
	public void act(MessageHandler handler) {		
		...
		handler.accept("gameList","clearGames",null);
		//Iterate through each of the element contained.
		for(int i=0;i<gameNames.size();++i){
			//Add one row of game info into the list of games.
			handler.accept("gameList","addGame",new String[]{gameNames.get(i),mapNames.get(i),colours.get(i)});
		}
	}
