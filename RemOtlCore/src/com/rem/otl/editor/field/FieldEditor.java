...
public class FieldEditor <SubjectType extends Object> extends MenuButton{
		...
		if(e.getAction()==ClickEvent.ACTION_UP){
			...
			if(!isWithin(e.getX(),e.getY())){
				if(onTypes.size()>0){
					...
					Hub.handler.removeOnType(onTypes.get(currentOnType));
				}
				else {
					Hub.handler.removeOnType(this.closerKeyListener);
				}
		...
		if(onTypes.size()>0){
			...
			closerKeyListener = new KeyBoardListener(){
				...
				public void onType(KeyBoardEvent event) {
					...
					Hub.handler.removeOnClick(self);
	...
	public void changeOnTypeTo(TextFieldComponent<SubjectType, ?> textField) {
		...
		Hub.handler.removeOnType(onTypes.get(currentOnType));
		Hub.handler.giveOnType(onTypes.get(i));
		currentOnType=i;
		if(currentOnType>=onTypes.size()){
			currentOnType=onTypes.size()-1;
		}
