...
import com.rem.otl.editor.field.BooleanOnClickFieldComponent;
...
import com.rem.otl.editor.program.ProgramSquareEditor;
...
import com.rem.otl.game.environment.onstep.OnStepAction;
import com.rem.otl.game.environment.onstep.OnStepSquare;
import com.rem.otl.game.environment.program.ProgramState;
import com.rem.otl.game.environment.program.ProgrammableSquare;
import com.rem.otl.game.environment.update.UpdatableSquare;
import com.rem.otl.game.environment.update.UpdateAction;
...
public abstract class Editor extends GraphicView {
	...
	protected java.util.Map<Integer,Action<ClickEvent>> modeOnClick = new HashMap<Integer,Action<ClickEvent>>();
	protected java.util.Map<Integer,Action<ClickEvent>> modeOnRelease = new HashMap<Integer,Action<ClickEvent>>();
	
	protected GraphicView parentView;


	public Editor(GraphicView parentView){
		super();
	...
	protected void setupModes(){
		modeOnClick.put(MODE_WAIT_FOR_RELEASE,new Action<ClickEvent>(){
			@Override
			public void act(ClickEvent e) {
			}});
		modeOnClick.put(MODE_NEUTRAL,new Action<ClickEvent>(){
			@Override
			public void act(ClickEvent e) {
				if(shape>0){
		...
		modeOnClick.put(MODE_MAKE_SQUARE,new Action<ClickEvent>(){
			public void act(ClickEvent e){
				builder1.resize(snapClickToGrid(e.getX()-builder1.getX(),Map.X_axis),
						snapClickToGrid(e.getY()-builder1.getY(),Map.Y_axis));
			}});
		modeOnRelease.put(MODE_NEUTRAL,new Action<ClickEvent>(){
			public void act(ClickEvent e){
			}
		});
		modeOnRelease.put(MODE_WAIT_FOR_RELEASE,new Action<ClickEvent>(){
			public void act(ClickEvent e){
				mode=MODE_NEUTRAL;
			}
		});
		final Editor editor = this;
		modeOnRelease.put(MODE_MAKE_SQUARE,new Action<ClickEvent>(){
			public void act(ClickEvent e){
				mode=MODE_NEUTRAL;
	...
	protected boolean handleButtons(ClickEvent e){
		for(int i=0;i<buttons.size();++i){
			...
			if(button.isVisible()&&button.isWithin(e.getX(), e.getY())){
				...
				else if(e.getAction()==ClickEvent.ACTION_UP){
	...
	public boolean onClick(ClickEvent e){		
		if(e.getButton()==ClickEvent.MOUSE_LEFT){
			if(mode==MODE_NEUTRAL){
				if(handleButtons(e))return true;
			}
			if(e.getAction()==ClickEvent.ACTION_DOWN&&modeOnClick.get(mode)!=null){
		...
		else if(e.getButton()==ClickEvent.MOUSE_RIGHT){
	...
	public boolean hoverOnSquare(Square square, HoverEvent e){
	...
	private void addFileToAction(OnStepAction action){
		Resource<InputStream> file = GetFileMenu.getFile(this,"maps",false);
		while(file==null){
			file = GetFileMenu.getFile(this,"maps",false);
		}
		String entity = new String(file.getPath());
		Hub.map.addEntity(Map.ID_NEXT_MAP,entity);
		action.setTarget(((Integer)Hub.map.getEntity(Map.ID_NEXT_MAP_SIZE))-1);
	}
	...
	protected float snapClickToGrid(float motionEventValue, boolean axis){
		int value = ((Map)Hub.map).getIntCoordinate(
				motionEventValue+((Map)Hub.map).getFloatCoordinate(granity,axis)/2f,axis);
		value= value - value%granity;
		if(value==0){
			value=granity;
		}
		return ((Map)Hub.map).getFloatCoordinate(value,axis);
	}
				...
				for(UpdateAction innerAction:action){
					...
					Button button = new Button("editor_update_icons",innerAction.getIndex(),
						...
						public void act(ClickEvent event) {
							...
							Hub.handler.giveOnClick(updatableSquareDataField);						
							Hub.handler.giveOnType(updatableSquareDataField.getDefaultKeyBoardListener());
						}
					});
	...
	private void addOnCreateButtonToSquare(final OnCreateSquare ocs){
		...
		final Button button = new Button("editor_special_icons",0,"Edit this square's OnCreate action.(opens a ocs file)",null,new ButtonAction(){
			...
			public void act(ClickEvent event) {
				...
				Resource<InputStream> saveTo = GetFileMenu.getFile(editor,"ocs",true);
	...
	private void addProgramEditorButtonToSquare(final ProgrammableSquare ps){
		...
		final Button button = new Button("editor_special_icons",1,
			...
			@Override
			public void act(ClickEvent event) {
				Hub.gui.setView(new ProgramSquareEditor(editor,ps));
			}
		},null);
