...
public class MapEditor extends Editor implements KeyBoardListener{
	...
	private com.rem.otl.game.environment.Map myLoadedMap;
	...
	public MapEditor(GraphicView parentView){
		...
		if(saveTo!=null){
			...
			myLoadedMap = (Map)Hub.map;
			squares = ((Map)Hub.map).getSquares();
			for(Square square:((Map)Hub.map).getSquares()){
	...
	public void saveCurrent(){
		...
		com.rem.otl.game.environment.Map map = com.rem.otl.game.environment.Map.createMap(myLoadedMap.getMapId());
		myLoadedMap.copyTo(map);
		for(Square square:squares){
			square.setRoot(this);
		}
		for(Square square:((Map)Hub.map).getTemplateSquares()){
			square.setRoot(this);
		}
		Storage.saveMap(Hub.manager.createOutputStream(saveTo.getPath(),IFileManager.ABSOLUTE), map);
		for(Square square:squares){
			square.reposition(square.getX()+screenX,
					square.getY()+screenY);
		}
